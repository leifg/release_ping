defmodule ReleasePing.Outgoing.Aggregates.SubscriptionTest do
  ...
  describe "notify subscriber" do
    ...
    test "succeeds with correct endpoint", %{
      bypass: bypass,
      aggregate: aggregate,
      signature: signature,
      subscription_added_event: subscription_added_event,
      ...
      Bypass.expect(bypass, "POST", "/", fn conn ->
        ...
        assert Plug.Conn.get_req_header(conn, "x-rp-webhook-uuid") == [
                 notify_subscriber_command.uuid
               ]

        assert Plug.Conn.get_req_header(conn, "x-rp-webhook-subscription-id") == [
                 subscription_added_event.uuid
               ]

        assert Plug.Conn.get_req_header(conn, "x-rp-webhook-session-id") == [
                 notify_subscriber_command.session_uuid
               ]
    ...
    test "fails with connection reset", %{
      bypass: bypass,
      aggregate: aggregate,
      signature: signature,
      notify_subscriber_command: notify_subscriber_command
