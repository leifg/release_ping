defmodule ReleasePing.Core.Aggregates.Software do
  ...
  def execute(%Software{uuid: nil}, %AddSoftware{} = add) do
    case validate_version_scheme(add.version_scheme) do
      {:error, {reason, position}} ->
        {:error, {:regex_error, "'#{reason}' at position #{position}"}}

      {:ok, version_scheme_regex} ->
        %SoftwareAdded{
          uuid: add.uuid,
          name: add.name,
          slug: add.slug,
  ...
  def execute(%Software{} = software, %PublishRelease{} = publish) do
    cond do
      is_nil(publish.version_string) ->
        nil

      MapSet.member?(software.existing_releases, publish.version_string) ->
        nil
      ...
      true ->
        ...
        display_version =
          EEx.eval_string(
            software.display_version_template,
            assigns: extended_version_info
          )
  ...
  def apply(%Software{} = software, %LicensesChanged{} = change) do
    %Software{software | licenses: change.licenses}
  end

  def apply(%Software{} = software, %VersionSchemeChanged{} = change) do
    %Software{software | version_scheme: change.version_scheme}
  end
